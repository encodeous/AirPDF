...
*   **Session Lifetime:** A `session_id` is created when the user explicitly initiates a connection. Automatic reconnects reuse that `session_id` as long as the session is re-established within a 1-minute timeout window. The iPad sends a `Heartbeat` message every 5 seconds to keep the QUIC connection alive; the Mac silently ignores these. If the QUIC connection drops and is not re-established within 30 seconds, the next connection attempt is treated as a new session and the iPad discards all cached document state before processing the fresh `PdfData` stream.
*   **iPad Backgrounding:** When the iPad app is backgrounded, iOS will suspend it and the QUIC connection will go silent. QUIC keepalive probes will eventually fail, and the Mac tears down the connection. When the iPad returns to the foreground, it detects the dead connection and initiates a reconnect. If within the 30-second window, the existing `session_id` is reused; otherwise a fresh session begins.
...
*   **Per-page canvas architecture:** `PDFPageOverlayViewProvider` (iOS 16+ / macOS 13+) provides a `PKCanvasView` overlay per page. PDFKit manages positioning, sizing, rotation, and lifecycle (recycling overlays as pages scroll off-screen). Each canvas holds only the `PKDrawing` for its page, loaded from `PdfData.page_drawings` on open. Drawing state is preserved across overlay recycling via a `pageDrawings: [Int: Data]` dictionary in the coordinator. The `PKToolPicker` is anchored to the hosting view controller (not to any canvas) so it remains visible regardless of overlay recycling. On the Mac, the same `PDFPageOverlayViewProvider` pattern is used with a read-only `NSView` overlay that renders `PKDrawing.image()` for display; annotations are only written at save time.
...
*   **Diffing:** Each page canvas maintains a cache of the previously-known set of `stroke_id`s. Upon a change, it identifies the delta (new strokes → `StrokeBatch`; removed strokes → `StrokeRemove`). When strokes are modified in-place (e.g. lasso move/transform), the differ detects the change and emits a `StrokeRemove` for all old strokes followed by a `StrokeBatch` for all new strokes.
...
All messages are framed as a `SyncEnvelope` protobuf message (timestamp + `Payload` oneof). The `Payload` oneof covers: `Hello`, `Welcome`, `Heartbeat`, `PdfData`, `StrokeBatch`, `StrokeRemove`, `PdfClose`, `Undo`, `Redo`, and `Error`. `StrokeRemove` carries `repeated string stroke_ids` to batch multiple removals in a single message (e.g. lasso move emits one `StrokeRemove` + one `StrokeBatch`). Liveness is maintained by the iPad sending a `Heartbeat` every 5 seconds; the Mac silently ignores it.
...
5.  Implement `Hello`/`Welcome` handshake with strict protocol version validation, single-client rejection, and reconnect logic that reuses `session_id` for up to 1 minute. After the handshake, the iPad sends a `Heartbeat` every 5 seconds to keep the QUIC connection alive; the Mac silently ignores it.
