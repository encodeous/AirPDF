...
*   **Undo/Redo:** Undo and Redo are **bidirectional global document-level actions** — either device can trigger them, and neither message carries a page index (the receiving side's own undo manager determines scope). The Mac maintains the authoritative global undo stack, which records all actions in order: stroke batches received from the iPad and document edits made on the Mac (text boxes, page insertions, etc.). Four cases arise:
    1. **iPad user undoes a stroke:** The iPad executes undo on its local `PKCanvasView` `UndoManager` immediately (for responsiveness); the resulting `PKDrawing` diff propagates to the Mac naturally as `StrokeRemove`. The iPad simultaneously sends `Undo` to notify the Mac to pop its global undo stack in sync.
    2. **Mac user undoes a stroke action:** The Mac inspects the top of its global undo stack, sees a stroke action, and sends `Undo` to the iPad. The iPad invokes its `PKCanvasView` `UndoManager`; the resulting diff propagates back as `StrokeRemove`.
    3. **iPad user triggers undo when the Mac's top action is non-stroke:** The iPad sends `Undo`; the Mac pops the non-stroke action, applies it locally (e.g., removes a text box), and re-sends `PdfData` with the updated document and current `page_drawings`. Nothing changes on the iPad's stroke canvas.
    4. **Mac user undoes a non-stroke action:** The Mac applies the undo locally and re-sends `PdfData`. No `Undo` message is sent to the iPad since the stroke canvas is unaffected.
    Redo follows the exact same four-case logic in reverse.
...
8.  Mac: Maintain the global document-level undo stack (covering both Mac document edits and iPad stroke batches). Implement `Undo`/`Redo` toolbar buttons: if the top of the stack is a stroke action send `Undo`/`Redo` to the iPad and await the diff; if non-stroke apply locally and re-send `PdfData`. Handle incoming `Undo`/`Redo` from the iPad by popping the global stack: if the top is a non-stroke action apply it and re-send `PdfData`; otherwise the stroke diff will arrive separately via `StrokeRemove`/`StrokeBatch`.
