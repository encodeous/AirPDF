...
All messages are framed as a `SyncEnvelope` protobuf message (timestamp + `Payload` oneof). The `Payload` oneof covers: `Hello`, `Welcome`, `Heartbeat`, `PdfData`, `StrokeBatch`, `StrokeRemove`, `PdfClose`, `Undo`, `Redo`, and `Error`. Liveness is maintained by the iPad sending a `Heartbeat` every 5 seconds; the Mac silently ignores it.
