...
*   **Undo/Redo:** PencilKit's `UndoManager` operates locally on the iPad. An undo/erase on the iPad produces a `StrokeRemove` diff naturally (the stroke disappears from `PKDrawing`, the diff detects it, and `StrokeRemove` is sent to the Mac). For Mac-initiated undo (e.g., a toolbar button), the Mac sends `Undo` (with `document_id` + `page_index`) to the iPad; the iPad invokes the page canvas's `UndoManager`, and the resulting `PKDrawing` change propagates back as `StrokeRemove`. This keeps the iPad's undo stack as the single source of truth, preventing stack corruption from independent Mac-side state changes.
...
8.  Mac: Implement `Undo`/`Redo` toolbar buttons that send the corresponding message to the iPad; handle the resulting `StrokeRemove`/`StrokeBatch` diff.
